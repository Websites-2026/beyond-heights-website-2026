...
const Donate: React.FC = () => {
  ...
  const handleScreenshotChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        ...
        // Keywords indicating a transaction reference or details
        const hasUpiOrTxn = /transaction|txn|tx\s?id|utr|ref|reference|upi|ptsbi|9363486261/.test(text);
        
        // Keywords indicating a successful payment status or amount/currency
        const hasPaymentStatus = /success|successful|completed|paid|transferred|sent|received|done|payment|status|debited|inr|rs|₹/.test(text);

        if (hasUpiOrTxn || hasPaymentStatus) {
          setIsScreenshotVerified(true);
        } else {
          setError('We could not verify this as a payment screenshot. Please ensure it shows the payment success screen with transaction details.');
        }
      } catch (err) {
        console.error('OCR Verification error:', err);
        // Fallback: in case Tesseract fails to load/run, do not block the user
        setIsScreenshotVerified(true);
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!screenshot) {
      setError('Please upload the payment screenshot to confirm your donation.');
      return;
    }

    if (!isScreenshotVerified) {
      setError('Please upload a valid payment confirmation screenshot.');
      return;
    }
    ...
    try {
      ...
      const screenshotMimeType = screenshot.type;
          ...
          <div className="lg:col-span-3 bg-[#111111] border border-neutral-800 rounded-2xl p-8 md:p-12">
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="space-y-2">
                ...
                <div
                  onClick={() => !loading && !isOcrLoading && fileInputRef.current?.click()}
                  className={`w-full bg-neutral-900 border-2 border-dashed rounded-lg p-8 flex flex-col items-center justify-center gap-3 cursor-pointer transition-all duration-300 ${
                    isScreenshotVerified ? "border-green-500/50 hover:border-green-500" : "border-neutral-700 hover:border-red-500/50"
                  }`}
                >
                  {screenshotPreview ? (
                    <div className="relative w-full max-w-xs flex flex-col items-center">
                      ...
                      {isOcrLoading && (
                        <div className="absolute inset-0 bg-black/60 rounded-lg flex flex-col items-center justify-center gap-2 text-white">
                          <Loader2 size={24} className="animate-spin text-red-500" />
                          <span className="text-[10px] font-black uppercase tracking-wider">Scanning screenshot...</span>
                        </div>
                      )}
                      {isScreenshotVerified && !isOcrLoading && (
                        <div className="mt-2 flex items-center gap-1.5 bg-green-500/10 border border-green-500/30 rounded-full px-3 py-1 text-green-400 text-[10px] font-black uppercase tracking-wider">
                          <Check size={12} />
                          Screenshot Verified
                        </div>
                      )}
